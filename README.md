# Maaz

Web Developer | Web3 Explorer | Hackathon Builder | Community Lead  
<a href="https://maazx.dev" target="_blank">Portfolio</a> • <a href="https://www.linkedin.com/in/maaz--/" target="_blank">LinkedIn</a> • <a href="https://x.com/Maaztwts" target="_blank">Twitter</a>

---

## About

Exploring tech through building. Started with Machine Learning, later shifted towards full-stack and blockchain development.  
Focused on building products, winning hackathons, and contributing to open source.

---

## Achievements

- 🥇 **1st Prize – GLM 4.7 Global Hackathon** (X Track) — FlickAI desktop AI assistant (₹2,27,500 / $2,500 + Cerebras Pro plan)
- 🥇 **1st Prize – CLI Hackathon 2025** — P2P file-sharing CLI tool (**₹1,05,000 / $1,200**)  
- 🥇 **1st Prize – Stellar India Ideathon 2025** — NFT-based rewards platform
- 🥇 **1st Prize – Stack 3.0 Hackathon** — ₹10,000 Amazon voucher + official merchandise
- 🥇 **1st Prize – Hack Beyond Limits 2025** — MSME logistics solution  
- 🥉 **3rd Prize – Stellar National Bounty 2025** (**$100 / 441 xlm**)  
- 🏅 **Top 4 – Monad Blitz Delhi Hackathon 2025**  (**Won F75 Aula Keyboard**)
- 🏅 **Top 7 (South & SEA) – TON Hackathon 2025**  
- 👥 **Technical Lead – Data Science Club (220+ members)**

---

## Tech Stack

**Languages:** JavaScript, Python, C++, C  
**Web:** React, Next.js, Tailwind, Flask  
**Web3:** Solidity, TON, Stellar, Soroban  
**AI/ML:** TensorFlow, Pandas, NumPy  
**Tools:** Git, Firebase, Vercel, Docker

---

## Focus

- Web & Web3 Development  
- Product Building & Hackathons  
- Community & Open Source  
- Applied ML Projects
